  ...
  return (
    <>
      <div className={cn(
        ...
        <div className="mt-auto pt-4 shrink-0 border-t border-zinc-200 dark:border-white/10 flex flex-col gap-1.5">
          {!isGold ? (
            <button 
              onClick={() => setIsUpgradeOpen(true)}
              className={cn(
                "flex items-center rounded-[16px] transition-all relative group border border-transparent w-full overflow-hidden",
                isExpanded ? "px-4 py-3 gap-4 justify-start" : "px-0 py-3 justify-center",
                "bg-gradient-to-r from-yellow-400/10 to-orange-500/10 hover:from-yellow-400/20 hover:to-orange-500/20 active:scale-[0.98]",
                "text-yellow-600 dark:text-yellow-400"
              )}
            >
              {isExpanded && <div className="absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-white/40 dark:via-white/20 to-transparent group-hover:animate-[shimmer_1.5s_infinite]" />}
              
              <Crown className="w-6 h-6 transition-transform shrink-0 group-hover:scale-110 group-hover:text-yellow-500 drop-shadow-sm" strokeWidth={2.5} />
              {isExpanded && <span className="font-bold text-[1rem] whitespace-nowrap tracking-wide">Upgrade Gold</span>}
            </button>
          ) : (
            <div 
              className={cn(
                "flex items-center rounded-[16px] transition-all relative w-full cursor-default",
                isExpanded ? "px-4 py-3 gap-4 justify-start" : "px-0 py-3 justify-center",
                "bg-gradient-to-r from-yellow-400 to-yellow-500 shadow-md border border-yellow-300",
                "text-white"
              )}
            >
              <Crown className="w-6 h-6 shrink-0 text-white drop-shadow-md" strokeWidth={2.5} />
              {isExpanded && <span className="font-bold text-[1rem] whitespace-nowrap tracking-wide drop-shadow-sm">Gold Member</span>}
            </div>
          )}
