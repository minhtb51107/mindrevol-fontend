...
export const MoodSelectionSheet: React.FC<Props> = ({ isOpen, onClose, onSubmit, currentMood }) => {
    const { icon, setIcon, message, setMessage, loading, handleSubmit } = useMoodForm(isOpen, onClose, onSubmit, currentMood?.icon, currentMood?.message, "😎");
                        ...
                        <div key={idx} className="mb-6">
                            ...
                            <div className="grid grid-cols-4 gap-3">
                                {cat.emojis.map((item, i) => (
                                    <div key={i} onClick={() => setIcon(item.icon)} className={`flex flex-col items-center justify-center p-3 rounded-2xl cursor-pointer transition-all ${icon === item.icon ? 'bg-white/30 border border-white/50 scale-105' : 'bg-white/10 hover:bg-white/20'}`}>
