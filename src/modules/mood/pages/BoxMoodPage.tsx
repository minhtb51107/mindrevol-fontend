                    ...
                    <div className="w-full max-w-[500px] bg-white dark:bg-[#2B2A29] rounded-[40px] shadow-[0_8px_32px_rgba(0,0,0,0.04)] border border-white/50 dark:border-[#3A3734] p-8 md:p-10 relative z-20 flex flex-col items-center text-center transition-all duration-500 hover:shadow-[0_16px_48px_rgba(0,0,0,0.08)]">
                        ...
                        {isViewingMe ? (
                                            ...
                                            {viewingMood.reactions.map(r => (
                                                <div key={r.userId} className="flex items-center gap-1.5 bg-[#F4EBE1]/50 dark:bg-[#1A1A1A] px-3 py-1.5 rounded-[12px] border border-transparent dark:border-white/5">
                                                    {r.avatarUrl ? (
                                                        ...
                                                        <div className="w-5 h-5 rounded-full bg-[#E2D9CE] dark:bg-[#3A3734] flex items-center justify-center text-[9px] font-black text-[#1A1A1A] dark:text-white">{r.fullname.charAt(0).toUpperCase()}</div>
                                                    )}
                            ...
                            <div className="flex flex-col items-center w-full animate-in fade-in zoom-in-95 duration-300">
                                <div className="text-[6rem] md:text-[8rem] leading-none mb-6 hover:scale-110 transition-transform cursor-pointer drop-shadow-xl">{viewingMood.icon}</div>
                                {viewingMood.message && <p className="text-[1.1rem] text-[#1A1A1A] dark:text-white font-extrabold mb-8 leading-relaxed">{viewingMood.message}</p>}
                                ...
                                <div className="flex items-center justify-center gap-3 mb-6 bg-[#F4EBE1]/50 dark:bg-[#1A1A1A] p-2 rounded-[24px]">
                                    {['👍', '❤️', '😂', '🥺'].map(emoji => {
                                        ...
                                        return (
                                            <button key={emoji} onClick={() => handleReact(viewingMood.id, emoji)} className={`w-12 h-12 rounded-[16px] text-[1.4rem] flex items-center justify-center transition-all active:scale-90 ${hasReacted ? 'bg-[#1A1A1A] text-white dark:bg-white dark:text-[#1A1A1A] shadow-[0_4px_12px_rgba(0,0,0,0.1)] -translate-y-1' : 'bg-white dark:bg-[#3A3734] hover:bg-white/80 shadow-sm border border-transparent dark:border-white/5'}`}>
                                                {emoji}
                                            </button>
                                        )
                                    })}
                                ...
                                <div className="w-full flex items-center bg-[#F4EBE1]/50 dark:bg-[#1A1A1A] rounded-[20px] p-1.5 border border-[#D6CFC7]/50 dark:border-[#3A3734]">
                                    <input type="text" value={replyMessage} onChange={(e) => setReplyMessage(e.target.value)} placeholder={`Gửi lời nhắn...`} className="flex-1 bg-transparent border-none px-4 text-[0.95rem] font-bold text-[#1A1A1A] dark:text-white placeholder:text-[#A09D9A] focus:ring-0" />
                                    <button disabled={!replyMessage.trim()} onClick={handleReply} className="w-10 h-10 rounded-[14px] bg-[#1A1A1A] dark:bg-white text-white dark:text-[#1A1A1A] flex items-center justify-center disabled:opacity-50 active:scale-95 transition-all hover:shadow-md"><Send size={18} className="-ml-0.5" strokeWidth={2.5} /></button>
                                </div>
                            </div>
                        ) : (
                            <div className="flex flex-col items-center animate-in fade-in zoom-in-95 duration-300">
                                ...
                                <p className="text-[0.95rem] text-[#8A8580] font-semibold mb-8 px-4">{viewingUser.fullname?.split(' ')[0]} chưa cập nhật cảm xúc. Hãy chọc xem sao!</p>
                                <button onClick={() => viewingUserId && handleAskMood(viewingUserId)} className="flex items-center gap-2 px-8 h-[56px] bg-[#1A1A1A] dark:bg-white text-white dark:text-[#1A1A1A] hover:bg-black rounded-[20px] text-[1.05rem] font-extrabold shadow-[0_8px_20px_rgba(0,0,0,0.15)] active:scale-95 transition-all group">
                    ...
                    <div className="flex gap-4 overflow-x-auto overflow-y-visible custom-scrollbar pt-6 pb-2 px-2 items-center">
                        <div className="flex flex-col items-center gap-2 cursor-pointer shrink-0 group" onClick={() => setViewingUserId(user?.id)}>
                            <div className={`w-[60px] h-[60px] rounded-[18px] flex items-center justify-center relative transition-all duration-300 ${viewingUserId === user?.id ? 'bg-[#1A1A1A] dark:bg-white shadow-[0_8px_20px_rgba(0,0,0,0.15)] -translate-y-3' : 'bg-[#F4EBE1] dark:bg-[#2B2A29] hover:bg-[#E2D9CE]'}`}>
                                {myMood ? <span className="text-[1.8rem] leading-none">{myMood.icon}</span> : user?.avatarUrl ? <img src={user.avatarUrl} alt="You" className="w-[calc(100%-6px)] h-[calc(100%-6px)] object-cover rounded-[14px]" /> : <span className="text-[#8A8580] font-black text-[1.2rem]">{user?.fullname?.charAt(0).toUpperCase()}</span>}
                            </div>
                            ...
                            return (
                                <div key={member.userId} className="flex flex-col items-center gap-2 cursor-pointer shrink-0 group" onClick={() => setViewingUserId(member.userId)}>
                                    <div className={`w-[60px] h-[60px] rounded-[18px] flex items-center justify-center relative transition-all duration-300 ${isSelected ? 'bg-[#1A1A1A] dark:bg-white shadow-[0_8px_20px_rgba(0,0,0,0.15)] -translate-y-3' : 'bg-[#F4EBE1] dark:bg-[#2B2A29] hover:bg-[#E2D9CE]'}`}>
                                        {friendMood ? (
                                            <><span className="text-[1.8rem] leading-none">{friendMood.icon}</span><div className="absolute -top-1 -right-1 w-3.5 h-3.5 bg-red-500 rounded-full border-2 border-white dark:border-[#121212] shadow-sm z-10 animate-pulse"></div></>
                                        ) : member.avatarUrl ? (
                                        ...
                                        {friendMood && (
                                            <div className="absolute -bottom-1.5 -right-1.5 w-6 h-6 rounded-[8px] border-[2px] border-white dark:border-[#121212] bg-[#E2D9CE] dark:bg-[#3A3734] flex items-center justify-center overflow-hidden shadow-sm">
                                                {member.avatarUrl ? <img src={member.avatarUrl} alt="avt" className="w-full h-full object-cover" /> : <span className="text-[10px] font-black text-[#1A1A1A] dark:text-white">{member.fullname?.charAt(0)}</span>}
                                            </div>
                                        )}
                                    </div>
                                    <span className={`text-[0.75rem] font-bold truncate max-w-[64px] ${isSelected ? 'text-[#1A1A1A] dark:text-white' : 'text-[#8A8580]'}`}>{member.fullname.split(' ')[0]}</span>
                                </div>
                            );
                        })}
                    </div>
