...
const safeFormatDate = (dateString: string | undefined | null, pattern: string) => {
  ...
  // [FIX] date-fns yêu cầu escape text bằng dấu nháy đơn
...
export const JourneyGalleryCard: React.FC<Props> = ({ journey, onCheckinClick }) => {
      ...
      <div className="flex items-center justify-between px-2 mb-4">
        ...
        >
            ...
            <p className="text-xs font-medium text-zinc-500 mt-1">
                {/* [FIX LỖI] Sửa format string: 'thg' được đặt trong dấu nháy đơn */}
                Bắt đầu {safeFormatDate(journey.startDate, "dd 'thg' MM, yyyy")}
            </p>
        </div>
        ...
        <button 
            onClick={() => navigate(`/journeys/${journey.id}`)}
            className="w-10 h-10 flex items-center justify-center rounded-full bg-zinc-900 border border-zinc-800 text-zinc-400 hover:text-white hover:bg-zinc-800 hover:scale-110 transition-all"
        >
            <ArrowRight className="w-5 h-5" />
        </button>
      </div>

      {/* --- SLIDER ẢNH (STYLE LOCKET) --- */}
      <div className="flex gap-4 overflow-x-auto pb-6 px-2 scrollbar-hide snap-x">
        
        {/* Nút Add Check-in (Luôn hiện đầu) */}
        <div 
            onClick={() => navigate(`/journeys/${journey.id}`)}
            ...
            journey.checkins.map((checkin) => (
                ...
                >
                    {/* STYLE LOCKET CHÍNH: 
                        ...
                        - rounded-[2rem] (Bo góc cực lớn)
                        - border-4 border-zinc-950 (Viền đen dày tạo độ tách biệt)
                    */}
                    <div className="w-full h-full rounded-[2rem] overflow-hidden border-4 border-zinc-950 shadow-lg relative z-10 transition-transform duration-200 group-hover:scale-95">
                        ...
                        {/* Gradient nhẹ ở đáy để làm nổi text */}
                        <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-60" />
                    </div>
                    
                    {/* Badge Ngày tháng (Nằm đè lên viền dưới) */}
