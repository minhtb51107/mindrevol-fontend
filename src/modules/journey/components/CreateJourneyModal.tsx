...
import { Loader2, Leaf, Star, Sparkles, ImagePlus, ChevronLeft, Check, ChevronDown, Package } from 'lucide-react';
...
const ALL_THEMES = Array.from({ length: 20 }, (_, i) => ({
  id: `bg-${i + 1}`,
  image: `/themes/bg (${i + 1}).jpg`
}));

const DEFAULT_THEMES = ALL_THEMES.slice(0, 4);
...
export const CreateJourneyModal: React.FC<Props> = ({ isOpen, onClose, onSuccess, defaultBoxId }) => {
  ...
  const handleCustomThemeUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      setValue('themeColor', URL.createObjectURL(file));
  ...
  return createPortal(
    ...
    >
      ...
      {/* [ĐÃ SỬA] Wrapper nội dung: Đổi cơ chế Flex để không bị cắt xén khi cuộn trên màn nhỏ */}
      <div className="relative min-h-full w-full flex flex-col items-center sm:py-12 pt-20 pb-4">
         {/* mt-auto và my-auto giúp tự động đẩy modal về đúng vị trí và cuộn an toàn */}
             ...
             {showAllThemes ? (
                 <div className="flex flex-col h-full animate-in slide-in-from-right-8 fade-in duration-300">
                     <div className="flex items-center gap-4 mb-6 shrink-0">
                         <button onClick={() => setShowAllThemes(false)} className="p-2 -ml-2 rounded-full hover:bg-zinc-100 dark:hover:bg-white/5 text-black dark:text-white transition-colors">
                             <ChevronLeft className="w-8 h-8" />
                         </button>
                         <h2 className="text-2xl text-black dark:text-white">Choose a Cover</h2>
                     </div>
                     
                     <div className="flex-1 pb-10">
                         <div 
                            onClick={() => fileInputRef.current?.click()}
                            className="w-full h-20 mb-6 rounded-2xl border-2 border-dashed border-zinc-300 dark:border-zinc-700 bg-white/50 dark:bg-[#1A1A1A]/50 hover:bg-white dark:hover:bg-[#222] cursor-pointer flex flex-col items-center justify-center text-zinc-500 dark:text-zinc-400 transition-colors active:scale-[0.98]"
                         >
                             <ImagePlus className="w-6 h-6 mb-1" />
                             <span className="text-sm">Upload custom cover</span>
                             <input type="file" accept="image/*" className="hidden" ref={fileInputRef} onChange={handleCustomThemeUpload} />
                         </div>
                         ...
                         <div className="grid grid-cols-3 gap-3">
                             {ALL_THEMES.map((theme) => (
                                 <div 
                                   key={theme.id}
                                   onClick={() => { setValue('themeColor', theme.image); setShowAllThemes(false); }}
                                   className={cn(
                                       "aspect-video rounded-[14px] overflow-hidden cursor-pointer border-[3px] transition-all hover:scale-[1.02]",
                                       watch('themeColor') === theme.image ? "border-green-500 shadow-md ring-2 ring-green-500/20" : "border-transparent opacity-80 hover:opacity-100"
                                   )}
                                 >
                 ...
                 <div className="flex-1 flex flex-col h-full relative z-10 animate-in slide-in-from-left-8 fade-in duration-300">
                    ...
                    <div className="bg-green-50 dark:bg-[#0D1711] rounded-[24px] p-5 mb-8 shadow-sm dark:shadow-none border border-green-100 dark:border-emerald-900/30 relative transition-colors duration-300">
                        <Leaf className="absolute right-4 bottom-14 w-12 h-12 text-green-200 dark:text-emerald-900/50 opacity-50 -rotate-12 pointer-events-none transition-colors" />
                        ...
                        <div className="flex justify-between items-end mb-2 relative z-10">
                            <h3 className="text-black dark:text-emerald-100 text-xl transition-colors">Theme</h3>
                            <button 
                                type="button" 
                                onClick={() => setShowAllThemes(true)}
                                className="text-[13px] text-green-600 dark:text-emerald-400 hover:underline font-sans font-medium"
                            >
                                More themes
                            </button>
                        </div>
                        <p className="text-lime-900/60 dark:text-emerald-200/50 text-xs mb-3 relative z-10 transition-colors">Sets the look of this journey</p>

                        <div className="flex gap-3 overflow-x-auto no-scrollbar mb-6 pb-1 relative z-10">
                            {DEFAULT_THEMES.map((theme) => (
                                <img 
                                key={theme.id}
                                src={theme.image}
                                alt="Theme preview"
                                onClick={() => setValue('themeColor', theme.image)}
                                className={cn(
                                    "w-20 h-14 rounded-xl shadow-sm shrink-0 cursor-pointer border-[3px] transition-all object-cover", 
                                    watch('themeColor') === theme.image ? "border-green-500 dark:border-emerald-400 scale-105 shadow-md" : "border-transparent opacity-80 hover:opacity-100" 
                                )} 
                                />
                            ))}
                        </div>
