...
const ProfilePage = () => {
  ...
  // [ĐÃ SỬA] Destructure thêm hàm toggleLocalVisibility
                      ...
                      {publicJourneys.length > 0 ? (
                        publicJourneys.map(journey => (
                          <JourneyGalleryCard 
                              ...
                              onVisibilityToggle={toggleLocalVisibility} // [ĐÃ SỬA]
                          />
                        ))
                      ) : (
                        ...
                        savedCheckins.map(checkin => (
                          <div key={checkin.id} className="aspect-square bg-zinc-100 dark:bg-zinc-800 rounded-2xl md:rounded-[24px] overflow-hidden cursor-pointer group relative border border-zinc-200 dark:border-zinc-700 shadow-sm" onClick={() => setSelectedCheckin(checkin)}>
                              {checkin.thumbnailUrl || checkin.imageUrl ? (
                                  <img src={checkin.thumbnailUrl || checkin.imageUrl} alt="Saved" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                              ) : (
                              ...
                              <div className="absolute top-2 right-2 p-1.5 md:p-2 bg-black/40 rounded-full backdrop-blur-md">
