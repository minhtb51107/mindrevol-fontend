// src/modules/call/components/GlobalCallOverlay.tsx
...
export const GlobalCallOverlay = () => {
    ...
    // THAM CHIẾU ÂM THANH (Bạn nhớ có file trong thư mục public/sounds/ nhé)
    ...
    useEffect(() => {
        if (incomingCall) {
            incomingAudio.current.play().catch(e => console.log("Auto-play prevented", e));
        } else {
            incomingAudio.current.pause();
        ...
        if (outgoingCall) {
            outgoingAudio.current.play().catch(e => console.log("Auto-play prevented", e));
        } else {
            outgoingAudio.current.pause();
    ...
    useEffect(() => {
        const sub = socket.subscribe('/user/queue/notifications', (noti: any) => {
            ...
            else if (noti.type === 'CALL_ACCEPTED') {
                // Người kia nhấc máy -> Tắt màn hình gọi đi -> Mở PiPWindow
            ...
            else if (noti.type === 'CALL_ENDED' || noti.type === 'MISSED_CALL') {
                clearCall();
            }
        });
        return () => {
    ...
    const handleAccept = async () => {
        ...
        try {
            ...
            // Xóa màn hình đổ chuông -> Hiện PiPWindow
        ...
        return (
            <div className="fixed inset-0 z-[99999] flex flex-col justify-between overflow-hidden bg-black animate-in fade-in duration-300">
                ...
                <div className="relative z-10 flex flex-col items-center pt-24">
                    <p className="text-white/70 text-lg font-medium mb-4 tracking-wide uppercase">
                        {incomingCall.callType === 'video' ? 'Cuộc gọi Video tới' : 'Cuộc gọi Thoại tới'}
                    </p>
                    <div className="relative mb-6">
                        <div className="absolute inset-0 bg-white/20 rounded-full animate-ping scale-150"></div>
                        <div className="absolute inset-0 bg-white/20 rounded-full animate-ping scale-110 delay-150"></div>
                ...
                <div className="relative z-10 flex justify-around items-end pb-20 px-8 w-full max-w-md mx-auto">
                    <div className="flex flex-col items-center gap-3">
                        <button onClick={handleReject} className="w-16 h-16 rounded-full bg-[#FF3B30] flex items-center justify-center hover:bg-[#FF3B30]/80 transition-all hover:scale-105 shadow-[0_8px_30px_rgba(255,59,48,0.4)]">
        ...
        return (
            <div className="fixed inset-0 z-[99999] flex flex-col justify-between overflow-hidden bg-black animate-in fade-in duration-300">
                ...
                <div className="relative z-10 flex flex-col items-center pt-24">
                    ...
                    <p className="text-white/60 mt-3 font-medium flex items-center gap-2">
                        Đang đổ chuông <span className="flex gap-1"><span className="animate-bounce delay-75">.</span><span className="animate-bounce delay-150">.</span><span className="animate-bounce delay-300">.</span></span>
                    </p>
                </div>

                <div className="relative z-10 flex justify-center pb-20 w-full">
                    <div className="flex flex-col items-center gap-3">
                        <button onClick={handleCancelOutgoing} className="w-16 h-16 rounded-full bg-[#FF3B30] flex items-center justify-center hover:bg-[#FF3B30]/80 transition-all hover:scale-105 shadow-[0_8px_30px_rgba(255,59,48,0.4)]">
