            ...
            <div className="w-full min-h-screen bg-gradient-to-b from-[#F4EBE1] to-[#FFFFFF] dark:from-[#121212] dark:to-[#0A0A0A] relative overflow-x-hidden transition-colors duration-500 pb-24 font-quicksand">
                <div className="max-w-[1440px] mx-auto px-5 md:px-10 relative z-10 pt-12 md:pt-14">
                    ...
                    <div className="relative w-full bg-gradient-to-br from-white/60 to-[#F4EBE1]/80 dark:from-[#1A1A1A]/80 dark:to-[#2B2A29]/50 backdrop-blur-md border border-white/50 dark:border-white/5 rounded-[32px] md:rounded-[40px] p-8 md:p-12 mb-10 overflow-hidden shadow-[0_8px_32px_rgba(0,0,0,0.04)] hover:shadow-[0_12px_40px_rgba(0,0,0,0.06)] transition-all duration-500">
                        ...
                        <img src="/moscow/moscow (16).png" alt="Moscow Mascot" className="absolute bottom-0 right-0 md:right-8 w-32 md:w-48 object-contain pointer-events-none opacity-90 translate-x-4 md:translate-x-0 translate-y-3 drop-shadow-xl" />
                        <div className="relative z-10 text-left w-full md:w-2/3">
                            <h1 className="text-[2.2rem] md:text-[2.8rem] font-black text-[#1A1A1A] dark:text-white mb-3 tracking-tight leading-tight drop-shadow-sm">Không gian của bạn</h1>
                            <p className="text-[1.05rem] font-bold text-[#8A8580] dark:text-[#A09D9A] leading-relaxed max-w-[400px] md:max-w-none">Nơi lưu giữ những mảnh ghép ký ức và chia sẻ cùng người thương.</p>
                        </div>
                    </div>
                        ...
                        <>
                                            ...
                                            {invitations.map(invite => (
                                                <div key={invite.id} className="bg-white/80 dark:bg-[#1A1A1A]/80 backdrop-blur-md shadow-[0_8px_24px_rgba(0,0,0,0.03)] border border-white/50 dark:border-white/5 rounded-[32px] p-6 flex flex-col justify-between transition-all hover:shadow-[0_16px_40px_rgba(0,0,0,0.06)] hover:-translate-y-1">
                                                    <div className="flex items-center gap-4 overflow-hidden mb-6">
                                                        ...
                                                        <div className="truncate">
                                                            <p className="text-[0.9rem] text-[#8A8580] dark:text-[#A09D9A] truncate mb-0.5 font-semibold">
                                                                <span className="font-extrabold text-[#1A1A1A] dark:text-white">{invite.inviterName}</span> mời
                                                            </p>
                                                    ...
                                                    <div className="flex items-center gap-3 shrink-0">
                                                        <button onClick={() => handleAcceptInvite(invite.id)} className="flex-1 h-12 rounded-[16px] bg-[#1A1A1A] text-white hover:bg-black dark:bg-white dark:text-[#1A1A1A] dark:hover:bg-gray-200 flex items-center justify-center transition-all active:scale-95 shadow-[0_8px_16px_rgba(0,0,0,0.1)]"><Check size={20} strokeWidth={3} /></button>
                                                        <button onClick={() => handleRejectInvite(invite.id)} className="flex-1 h-12 rounded-[16px] bg-[#F4EBE1]/50 dark:bg-[#2B2A29] text-[#1A1A1A] dark:text-white hover:bg-[#E2D9CE] hover:text-red-500 border border-transparent hover:border-red-100 flex items-center justify-center transition-all active:scale-95"><X size={20} strokeWidth={3} /></button>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
                            ...
                            {activeTab !== 'invitations' && (
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 pb-16 animate-in fade-in slide-in-from-top-4">
                                    ...
                                    <button onClick={() => setIsCreateModalOpen(true)} className="group w-full aspect-[7/4] rounded-[32px] border-2 border-dashed border-[#D6CFC7] dark:border-[#3A3734] hover:border-[#1A1A1A] dark:hover:border-white bg-white/40 dark:bg-[#1A1A1A]/40 hover:bg-white/80 dark:hover:bg-[#1A1A1A]/80 transition-all duration-300 flex flex-col items-center justify-center gap-4 relative z-10 backdrop-blur-sm active:scale-[0.98]">
                                        <div className="w-14 h-14 bg-[#1A1A1A] dark:bg-white rounded-[20px] flex items-center justify-center text-white dark:text-[#1A1A1A] shadow-[0_8px_20px_rgba(0,0,0,0.15)] group-hover:-translate-y-1 transition-transform duration-300"><Plus size={28} strokeWidth={3} /></div>
                                        <div className="flex flex-col items-center">
                                            <span className="text-[#1A1A1A] dark:text-white text-[1.2rem] font-black">Tạo Không gian</span>
                                            <span className="text-[#8A8580] dark:text-[#A09D9A] text-[0.9rem] font-bold mt-0.5">Mở khu vực riêng của bạn</span>
                                        </div>
                                    </button>
