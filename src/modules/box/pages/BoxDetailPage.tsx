...
import { MOOD_DICTIONARY } from '@/modules/mood/constants/moodAssets';
...
const BoxDetailPage: React.FC = () => {
    ...
    return (
        <MainLayout>
            ...
            <div id="box-detail-scroll" className="w-full h-full overflow-y-auto overflow-x-hidden bg-white dark:bg-[#121212] relative pb-24 transition-colors duration-500 font-quicksand">
                ...
                <div className="relative z-10 max-w-[1024px] mx-auto px-5 md:px-8">
                    
                    <div className="mt-8 md:mt-12 mb-12 md:mb-16 grid grid-cols-2 gap-5 md:gap-8">
                        <div className="flex flex-col w-full aspect-square md:aspect-[4/3]">
                            ...
                            <div className="flex-1 min-h-0 w-full bg-white/60 dark:bg-[#1A1A1A]/60 backdrop-blur-md rounded-[28px] md:rounded-[36px] shadow-[0_8px_24px_rgba(0,0,0,0.03)] border border-gray-100 dark:border-white/5 p-3 md:p-4 relative flex flex-col hover:shadow-[0_16px_32px_rgba(0,0,0,0.06)] transition-all duration-300">
                                <div className="w-full h-full rounded-[20px] md:rounded-[28px] overflow-hidden relative shadow-inner border border-black/5 dark:border-white/5">
                                    {/* FIX: Thêm prop mapMode bắt buộc */}
                                    <JourneyMap boxId={box.id} mapMode="markers" className="w-full h-full rounded-[inherit] overflow-hidden relative z-0" />
                                </div>
                                    ...
                                    {moods.length > 0 ? (
                                        <>
                                            {moods.slice(1, 5).map((m, idx) => {
                                                ...
                                                const sizes = ["w-10 h-10", "w-9 h-9", "w-8 h-8", "w-7 h-7"];
                                                const friendAsset = MOOD_DICTIONARY[m.icon] || MOOD_DICTIONARY['default'];
                                                return (
                                                    <div key={m.id} className={`absolute ${positions[idx]} ${sizes[idx]} opacity-70 animate-bounce group-hover:scale-110 transition-transform drop-shadow-sm`} style={{ animationDelay: `${idx * 0.3}s`, animationDuration: '3s' }}>
                                                        <img src={friendAsset.gif} alt="mood" className="w-full h-full object-contain" />
                                                    </div>
                                                );
                                            })}
                                            <div className="z-10 bg-white/80 dark:bg-black/40 p-4 rounded-full backdrop-blur-md shadow-md group-hover:scale-110 transition-transform duration-300 relative">
                                                <div className="w-16 h-16 drop-shadow-xl">
                                                    <img src={(MOOD_DICTIONARY[latestMood?.icon || 'default'] || MOOD_DICTIONARY['default']).gif} alt="latest mood" className="w-full h-full object-contain" />
                                                </div>
                                                <img src={latestMood?.avatarUrl} className="absolute -bottom-1 -right-1 w-6 h-6 rounded-full border-2 border-white dark:border-[#121212] object-cover bg-gray-100" />
                                            </div>
                                        </>
                                    ) : (
                                        ...
                                        <div className="flex flex-col h-full justify-between">
                                            <div className="flex items-center justify-between">
                                                <div className="flex items-center gap-3">
                                                    <img src={latestMood.avatarUrl} className="w-12 h-12 rounded-[16px] object-cover bg-gray-100 shadow-sm" />
                                                ...
                                                {moods.length > 1 && (
                                                    <div className="flex -space-x-3 drop-shadow-sm">
                                                        {moods.slice(1, 4).map(m => {
                                                            const asset = MOOD_DICTIONARY[m.icon] || MOOD_DICTIONARY['default'];
                                                            return (
                                                            <div key={m.id} className="w-9 h-9 rounded-full border-[3px] border-white dark:border-[#1A1A1A] bg-gray-100 flex items-center justify-center relative overflow-hidden">
                                                                <img src={m.avatarUrl} className="w-full h-full object-cover opacity-60" />
                                                                <img src={asset.gif} className="absolute w-5 h-5 object-contain drop-shadow-md z-10" />
                                            ...
                                            <div className="flex items-center gap-6 my-auto ml-2">
                                                <div className="w-24 h-24 drop-shadow-xl group-hover:scale-110 group-hover:-rotate-6 transition-transform duration-500">
                                                    <img src={(MOOD_DICTIONARY[latestMood?.icon || 'default'] || MOOD_DICTIONARY['default']).gif} alt="mood" className="w-full h-full object-contain" />
                                                </div>
                                                
                                                {latestMood.message && (
                                                    <div className="flex-1 bg-gray-100/80 dark:bg-[#2B2A29] p-4 rounded-[20px] rounded-tl-sm text-[1rem] font-extrabold text-[#1A1A1A] dark:text-white relative shadow-sm">
                                                        <div className="absolute -left-2 top-0 w-4 h-4 bg-gray-100/80 dark:bg-[#2B2A29]" style={{ clipPath: 'polygon(100% 0, 0 0, 100% 100%)' }}></div>
                                            ...
                                            {latestMood.userId !== user?.id ? (
                                                <div className="flex items-center gap-2.5 bg-white/50 dark:bg-[#121212]/50 backdrop-blur-md p-1.5 rounded-[20px] w-max">
                                                    {['👍', '❤️', '😂', '🥺'].map(emoji => (
                                                        <button key={emoji} onClick={(e) => { e.stopPropagation(); handleReact(latestMood.id, emoji); }} className="w-10 h-10 rounded-[14px] bg-white dark:bg-[#3A3734] hover:bg-gray-100 hover:-translate-y-1 transition-all flex items-center justify-center text-[1.2rem] shadow-[0_2px_8px_rgba(0,0,0,0.05)] border border-transparent dark:border-white/5">
                                                            {emoji}
                                                        </button>
                                                    ))}
